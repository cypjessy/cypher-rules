...
export default function WatchLiveModal({ isOpen, onClose }: WatchLiveModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/85 backdrop-blur-md p-2 sm:p-4 flex justify-center items-center">
      <div className="relative w-full max-w-5xl bg-[#0d001a] border border-brand-gold/20 rounded-2xl overflow-hidden shadow-2xl flex flex-col lg:flex-row h-[90vh] lg:h-[70vh] my-auto">
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-white hover:text-brand-gold transition-colors p-2 z-50 bg-black/50 backdrop-blur-md rounded-full border border-white/10"
        >
          <X className="w-5 h-5" />
        </button>
